...
If this alternative distribution would actually be true, deciding to reject the null would be a good decision. Though, we can also see that if this alternative is true. If the parameter really is .2, getting a bag with 0 or 10 yellow candies also does not happen that often. The probabilities for 10 yellow candies is almost none, and the probability for getting 0 yellow candies is about 11%. This means that if the alternative hypothesis is true, we would only correctly reject the null hypothesis in 11% of the cases. This is the power of the test.
