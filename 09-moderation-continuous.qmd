...
The PROCESS macro does not like long variable names, so we recommend to use short variable names in the data file. If you have long variable names, you can change them in the SPSS data editor. You can also accepting the risk of incorrect output by indicating this under `long variable names`.
...
{{< video https://youtu.be/kSqiAhGup0o?si=5k_wn6Uo_9cjmR-Y
    width="100%"
    height="315" 
>}}

Running a moderation analysis with PROCESS in SPSS.
:::
...
The higher order unconditional interaction effect is reported in the `Test(s) of highest order unconditional interaction(s)` section. This is the F-test for the interaction effect, which tests the proportion explained variance by the interaction to the total amount of variance in the model.
...
For this example we again use the PROCESS macro for SPSS. Installation instructions can be found in @sec-installPROCESS. We use the [smokers.sav](data/smokers5.sav) data file for this analysis. We will look at the effect of campaign exposure on attitude towards smoking moderated by the number of contacts that people have with smokers and their own smoking status being non-smoker, former smoker or smoker.
...
Finally, we click `OK` to run the analysis. The output will contain the regression coefficients for the predictor, moderator, and interaction variable, as well as the Johnson-Neyman output for the moderator. Video XX will show how to run the analysis in SPSS.
