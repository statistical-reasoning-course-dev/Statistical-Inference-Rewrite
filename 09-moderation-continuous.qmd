...
## Numerical Moderator {#sec-cont-moderator-regression}
...
#### SPSS manually mean center a variable

@vid-SPSSregcenter shows how to mean-center a variable in SPSS. The example data set is smokers.sav, which contains the effect of campaign exposure on attitude towards smoking moderated by contacts that people have with smokers.
...
If we want to interpret the conditional effect of contact on attitude ($b_2$), we must realize that this is the effect for people who score zero on the exposure variable if the exposure by contact interaction is included in the regression model. This is clear if we rearrange the regression equation as in @eq-contactbyexposure.
...
# This is old code that created a table for the moderation that also contained a covariate. I took out the covariate to make things a little more digestable SK
...
smokers$contact_mc <- smokers$contact - mean(smokers$contact)
# Unstandardized linear model.
model_1 <- lm(attitude ~ exposure_mc*contact_mc, data = smokers)
# Table with results in SPSS style.
results <- coef(summary(model_1))
# Adjust parameter names
attributes(results)$dimnames[[1]][1] <- "(Constant)"
...
attributes(results)$dimnames[[1]][3] <- "Contact (mean-centered)"
# attributes(results)$dimnames[[1]][4] <- "Status (smoker)"
attributes(results)$dimnames[[1]][4] <- "Exposure*Contact (mean-centered)"
# Confidence intervals
ci <- confint.lm(model_1)
results <- cbind(results, ci)
# Correctly standardized coefficients.
...
model_2 <- lm(z_attitude ~ z_exposure + z_contact + z_expocontact, data = smokers)
results_2 <- coef(summary(model_2))
results <- cbind(results[, 1:2], results_2[, 1], results[, 3:6])
results[1, 3] <- NA
# Set column names.
...
Looking at @tbl-cont-moderator-output, we see that the regression coefficient for the interaction effect of exposure and contact is not significantly different from zero. This means that the effect of exposure on attitude is not moderated by contact with smokers. The effect of exposure on attitude is the same for people who have more contact with smokers.
...
### Effect at different moderator values
...
With a numerical moderator we can also do this if we group moderator scores. Hainmueller et al. [-@RefWorks:3838] recommend creating three groups, each containing one third of all observations. These low, medium, and high groups correspond more or less with the minus one standard deviation/mean/plus one standard deviation values that we used for visualizing and testing conditional effects.
...
The video below (@vid-SPSSregSupport2) shows how to check common support of the predictor variable in the data for three groups of the moderator variable. 
...
### SPSS moderation analysis in PROCESS

As our previous example for the effect of exposure on attitude towards smoking moderated by contact with smokers did not show a significant interaction effect in @tbl-cont-moderator-output, we will look at another example for running the moderation analysis in SPSS using the PROCESS macro.
...
For this example we again use the PROCESS macro for SPSS. Installation instructions can be found in @sec-installPROCESS. We use the [agression.sav](data/agression.sav) data file for this analysis.
...
Under `Options`, we select `Generate data to visualize moderation` with `Model estimates only`, we can `Mean center components of products` for `Only continuous components`, and we need to `Probe interaction` with `Moments` to get the conditional effects for moderator values at 1 standardeviation below, above and at the mean, and tick the `Johnson-Neyman technique` box. Click `continue` to set these options.
...
Finally, we click `OK` to run the analysis. The output will contain the regression coefficients for the predictor, moderator, and interaction variable, as well as the Johnson-Neyman output for the moderator. @vid-RunModerationWithProcess below shows how to run the analysis in SPSS.

::: {#vid-RunModerationWithProcess}

{{< video https://youtu.be/UjzwBtRcy8g
    width="100%"
    height="315" 
>}}

Running a moderation analysis with a continuous moderator in SPSS with PROCESS 5.
:::


PROCESS produces the output in plain text. It covers the `Model Summary`, the `Model`, a seperate ANOVA test for the interaction, the conditianal effects of the predictor at one standard deviation below the mean, at the mean, and one standard deviation above the mean of the moderator, and the Johnson-Neyman output. @vid-InterpretModerationWithProcess shows you how to interpret the output of the PROCESS macro in SPSS.

::: {#vid-InterpretModerationWithProcess}
{{< video https://youtu.be/XqmmPGWANhg?si=W5GhDjbVSlZzRjBR
    width="100%"
    height="315" 
>}}

Interpreting a moderation analysis with PROCESS in SPSS.
:::
...
## Numerical and categorical moderators {#sec-RegressionContModSPSS}
...
# Create coordinates for the variable names.
variables <- data.frame(x = c(0.32, 0.41, 0.5, 0.68), 
                        y = c(.1, .3, .3, .1),
                        hjust = c(1, 0.5, 0.5, 0),
                        label = c("Exposure", "Contact", "Smoking status", "Attitude"))
ggplot(variables, aes(x, y)) + 
  # exposure effect
  geom_segment(aes(x = x[1], y = y[1], xend = x[4], yend = y[1]), arrow = arrow(length = unit(0.04, "npc"), type = "closed")) +
  # contact moderation
...
### SPSS numerical and categorical moderator with PROCESS

For this example we again use the PROCESS macro for SPSS. Installation instructions can be found in @sec-installPROCESS. We use the [smokers.sav](data/smokers5.sav) data file for this analysis. We will look at the effect of campaign exposure on attitude towards smoking moderated by the amount of contact that people have with smokers and their own smoking status being non-smoker, former smoker or smoker.

In SPSS we go to `Analyze > Regression > PROCESS`, in the PROCESS dialog, we select `Model 2` for a moderation model with two moderators and a second order interaction. We select `exposure` as the predictor `X variable`, and `attitude` as the dependent `Y variable`. We set `contact` as the continuous numerical `Moderator variable W`, and `status3` as the `Moderator variable Z`. The variable `status3` is a categorical variable with three levels: non-smoker (0), former smoker (1), and smoker (2). The PROCESS macro will create dummy variables for the categorical moderator, but we do have to indicate that this variable is categorical by clicking the `Multicategorical` butten and ticking the `Multicategorical` for `Variable Z`. You can leave the `Combo Box` at `Indicator/dummy`.

Under `Options`, we select `Generate data to visualize moderation` for `Model estimates only`, we can mean center our variables by selecting `Mean center components of products` for `Only continuous components`, and we need to `Probe interaction` with `Moments` to get the conditional effects for moderator values at 1 standardeviation below, above and at the mean, and tick the `Johnson-Neyman technique` box. Click `continue` to set these options.

The PROCESS macro does not like long variable names, so we recommend to use short variable names in the data file. If you have long variable names, you can change them in the SPSS data editor. You can also accept the risk of incorrect output by indicating this under `long variable names`.

Finally, we click `OK` to run the analysis. The output will contain the regression coefficients for the predictor, moderator, and interaction variable, as well as the Johnson-Neyman output for the moderator. @vid-SPSSrunModContCat will show how to run the analysis in SPSS.

::: {#vid-SPSSrunModContCat}

{{< video https://youtu.be/oOroCUYD1qk
    width="100%"
    height="315" 
>}}

Moderation analysis with a continuous and a categorical moderator in SPSS using the PROCESS macro.
:::
...
The output for visualizing the conditional effect of the focal predictor is similar to the previous example. The text can be copied to a syntax file, to create a scatterplot with the predictor on the x-axis, the dependent variable on the y-axis for each level of the categorical moderator. This is shown in @vid-SPSSintModContCat below.


::: {#vid-SPSSintModContCat}

{{< video https://youtu.be/kTJW7PVyB8U
    width="100%"
    height="315" 
>}}

Interpreting the results of a moderation analysis with a continuous and a categorical moderator in SPSS using the PROCESS macro.
:::
...
## Reporting Results {#sec-reportmoderation}
...
#| fig-cap: "The effect of exposure on attitude towards smoking."
#| fig-subcap: 
#|   - "Effects for groups with different smoking status (at average contact with smokers)."
#|   - "Effects at different levels of contact with smokers (effects for non-smokers)."
#| echo: false
#| warning: false
#| layout-ncol: 2
...
Communicate the results of a numerical moderator in a scatterplot with regression lines for the effect of the predictor at three moderator values: the mean value of the moderator, one standard deviation below the mean and one standard deviation above the mean (@fig-nummoderatorplot). The PROCESS macro provides the syntax for these plots.
