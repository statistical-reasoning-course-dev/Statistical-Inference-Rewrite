...
:::{#fig-mean-centering-plot}
```{r mean-centering-plot, eval=TRUE, echo=FALSE, fig.pos='H', out.width="640px", fig.align='center'}
#create dataframe
set.seed(4932)
...
ggplot(df) +
  ...
  theme_bw()
```
Histograms of the original contacts with smokers variable and the mean-centered variable. The red lines represent the means.
:::
...
:::{#fig-moderator-symmetry}
```{r moderator-symmetry, echo=FALSE, fig.pos='H', fig.align='center', fig.asp=0.2}
...
rm(variables)
```
Two conceptual diagrams of moderation for the same interaction effect.
:::
...
::: {#fig-continuous-interaction-visualization}
...
:::{#fig-cont-moderator-support}
```{r cont-moderator-support, echo=FALSE, message=FALSE, warning=FALSE, fig.pos='H', fig.align='center', out.width="640px"}
...
rm(smokers)
```
Common support of the predictor variable (exposure) at three levels of the moderator variable (contact).
:::
...
:::{#fig-report-moderator-visual}
```{r report-moderator-visual, fig.pos='H', fig.align='center', out.width='50%', fig.asp=1, fig.show='hold', echo=FALSE}
...
```
The effect of exposure on attitude towards smoking. Left: Effects for groups with different smoking status (at average contact with smokers). Right: Effects at different levels of contact with smokers (effects for non-smokers).
:::
