...
```{block2, type='rmdquestion', echo = Qch3}
1. What is the average candy weight in the population of candies? [<img src="icons/2answer.png" width=115px align="right">](#answer3.2.1){.buttonToAnswer}{.buttonToAnswer}
```

<A name="question3.2.2"></A>
```{block2, type='rmdquestion', echo = Qch3}
2. Move the slider until you have found the interval containing 95% of all samples that are closest to the (true) population value. What are the upper and lower limits of the interval that contains these samples? [<img src="icons/2answer.png" width=115px align="right">](#answer3.2.2){.buttonToAnswer}{.buttonToAnswer}
```
...
```{block2, type='rmdquestion', echo = Qch3}
1. How does the precision (width) of the interval estimate (represented by the double-sided arrow) change if you change the confidence level? Check what happens if you change the confidence level slider in Figure \@ref(fig:interval-level). [<img src="icons/2answer.png" width=115px align="right">](#answer3.3.1){.buttonToAnswer}
```

<A name="question3.3.2"></A>
```{block2, type='rmdquestion', echo = Qch3}
2. What happens if we want to be 100% certain that our interval contains the average candy weight of the next sample that we will draw? [<img src="icons/2answer.png" width=115px align="right">](#answer3.3.2){.buttonToAnswer}
```
...
```{block2, type='rmdquestion', echo = Qch3}
3. How does the precision of the interval estimate change if you change the size of the sample? Check what happens if you change the sample size slider. [<img src="icons/2answer.png" width=115px align="right">](#answer3.3.3){.buttonToAnswer}
```

<A name="question3.3.4"></A>
```{block2, type='rmdquestion', echo = Qch3}
4. How does the shape of the sampling distribution change if you change sample size? Explain what this means for the values of the sample statistic. [<img src="icons/2answer.png" width=115px align="right">](#answer3.3.4){.buttonToAnswer}
```
...
```{block2, type='rmdquestion', echo = Qch3}
5. How does the standard error depend on the size of the sample? Play around with the sample size slider in Figure \@ref(fig:se-point-est) to find the answer. [<img src="icons/2answer.png" width=115px align="right">](#answer3.3.5){.buttonToAnswer}
```

<A name="question3.3.6"></A>
```{block2, type='rmdquestion', echo = Qch3}
6. How does the shape of the sampling distribution express the standard error? Can you explain why the standard error, which is the standard deviation of the sampling distribution, affects the shape of the sampling distribution in this way? [<img src="icons/2answer.png" width=115px align="right">](#answer3.3.6){.buttonToAnswer}
```
...
```{block2, type='rmdquestion', echo = Qch3}
1. How do the two horizontal axes tell you the size of the standard error in grams? [<img src="icons/2answer.png" width=115px align="right">](#answer3.4.1){.buttonToAnswer}
```

<A name="question3.4.2"></A>
```{block2, type='rmdquestion', echo = Qch3}
2. How do you expect the location of the vertical lines to change if you change the size of the standard error? Check your expectation by using the slider. [<img src="icons/2answer.png" width=115px align="right">](#answer3.4.2){.buttonToAnswer}
```
...
In the candy weight example, we first need to know the mean of population candy weight, then we can construct a theoretical probability distribution of sample means (Figure \@ref(fig:normal-approximation)). But we do not know the population mean; we want to estimate it.
...
knitr::include_app("https://sharon-klinkenberg.shinyapps.io/pop-means-ci/", height="310px")
```

<A name="question3.5.1"></A>
```{block2, type='rmdquestion', echo = Qch3}
1. Click repeatedly on Figure \@ref(fig:pop-means-ci) to find the highest and lowest value of the population mean for which the sample mean is in the interval of sample means that have 95% probability of occurring. [<img src="icons/2answer.png" width=115px align="right">](#answer3.5.1){.buttonToAnswer}
```
...
What does this arrow represent and why does it carry this label? [<img src="icons/2answer.png" width=115px align="right">](#answer3.5.2){.buttonToAnswer}
```

<A name="question3.5.3"></A>
```{block2, type='rmdquestion', echo = Qch3}
3. How does the _z_ value of the sample mean help you to minimize the number of clicks you need? [<img src="icons/2answer.png" width=115px align="right">](#answer3.5.3){.buttonToAnswer}
```

<A name="question3.5.4"></A>
```{block2, type='rmdquestion', echo = Qch3}
4. How does the depicted interval estimate help you to minimize the number of clicks you need? [<img src="icons/2answer.png" width=115px align="right">](#answer3.5.4){.buttonToAnswer}
```

<A name="question3.5.5"></A>
```{block2, type='rmdquestion', echo = Qch3}
5. What is the most efficient strategy (minimum number of clicks) to determine the lower and upper limits of the population means for which the sample mean is among the 95% most likely samples? Explain why this is the most efficient strategy. [<img src="icons/2answer.png" width=115px align="right">](#answer3.5.5){.buttonToAnswer}
```

Recall that before section 3.5, we were reasoning from population mean to sampling distribution of sample means, then to a single sample mean. Based on the known population mean and standard error, we can draw an interval estimate around the population mean on sampling distribution (Figure 3.5). 95% of all sample means will fall into this interval estimate around the known population mean.

In reality, we do not know the population mean. We only know one sample mean from the data we currently collect. How can we estimate the population mean? 

We reserve what we did: instead of checking whether a sample mean is inside or outside of the interval estimate around the population mean (refer to 3.4), we shift the same interval estimate around that one sample mean, and check whether the interval estimate around this sample mean catches the population mean. These two ways of checking are equivalent. But the second way does not require us to know the population mean anymore if we replicate data collection many times. This is because if we shift the interval estimate around sample means, 95 percent of times, these interval estimates around sample means will catch the population mean, regardless of the population mean is known or not. We call the interval estimate around a sample mean _confidence interval_.

The Figure 3.7(new) illustrates the reasoning from sample means to sampling distribution of sample means. The Figure 3.7 starts with one time data collection of 30 samples drawn from a population distribution (the top plot). From these 30 samples, we can calculate a sample mean and its 95% confidence interval around it. 

This can be illustrated with the first data collection: average candy weight in 30 samples is 2.95 grams and the lower and upper boundary for 95% confidence interval are 2.59 grams and 3.32 grams. We use a vertical dash line here to indicate that we pretend to know this population mean, which is equal to 3 grams, but in reality, we do not know. We can see in the middle plot, this 95% confidence interval catches the blue dash line, aka, the population mean.

Let’s increase the number of times of data collection (replications!). Again, in each data collection, samples are drawn and recorded, the sample mean and 95% confidence interval around the sample mean are calculated. 

With more replications, the middle plot now shows how many 95% confidence intervals catch the population mean (indicated with green lines if yes, red lines if no). More times you replicate data collection, the percentage of all 95%CIs that catch the population mean gets closer to 95%. 

Let’s also increase sample size for each data collection. We see that in the middle plot all confidence intervals become narrower and, in the bottom plot the sampling distribution of sample means (the shape with green line) also becomes narrower. This sampling distribution is theoretically approximated by a normal distribution whose mean is the population mean and standard deviation is standard error.

The bottom plot also shows the histogram which consists of sample means from replications. Each sample mean comes from one replication. As the number of replications and sample size increase, the shape of histogram gets closer to the shape of theoretical sampling distribution of sample means (the shape with green line). 

We are using the normal distribution to approximate the sampling distribution of the sample means. To make our life easier, we can convert the sampling distribution, which is a normal distribution, to the standard normal distribution. How? We convert every sample mean to a z score by substracting the mean of sampling distribution of sample means (this is not in the vicious circle if we collect data for many times) from it, then divided by standard error. The critical z value 1.96 and -1.96 together marks the upper and lower limit of the interval containing 95% of all samples with means closest to the population mean. The critical z value 1.65 and -1.65 together marks the upper and lower limit of the interval containing 90% of all samples with means closest to the population mean. 

As a consequence, we are able to calculate 95% confidence interval around a sample mean by adding and subtracting 1.96 standard errors from that sample mean. This is much more efficient than selecting a lot of imaginary population means! 
...
Haven't we seen this calculation before? Yes we did, in Section \@ref(int-est-sample-mean), where we estimated the interval around population mean for sample means. We now simply reverse the calculation, using the interval of sample mean to estimate the population mean instead of the other way around.

### One confidence interval does not say anything {#conf-interval}
...
```{block2, type='rmdquestion', echo = Qch3}
1. Download the data set [candies.sav](https://shklinkenberg.github.io/Statistical-Inference/data/candies.sav) and use SPSS to calculate the 95% and 99% confidence intervals of average candy weight. 
    ...
    Interpret the results and explain why the 99% confidence interval is wider than the 95% confidence interval. [<img src="icons/2answer.png" width=115px align="right">](#answer3.6.1){.buttonToAnswer}
```
...
2. Let SPSS calculate the 95% confidence interval for median candy weight. Interpret the result. The data are in "candies.sav". 
    ...
    Remember the SPSS exercises in Section \@ref(boot-spss). [<img src="icons/2answer.png" width=115px align="right">](#answer3.6.2){.buttonToAnswer}
```
...
3. Use SPSS to determine the 95% confidence interval for a paired-samples _t_ test on candy colour fading under sunlight (variables colour_pre and colour_post in "candies.sav"). In your interpretation of the confidence interval, clarify the meaning of the statistic for which the confidence interval was calculated.
    ...
    The paired-samples _t_ test is available in SPSS under *Analyze > Compare Means*. [<img src="icons/2answer.png" width=115px align="right">](#answer3.6.3){.buttonToAnswer}
```
...
4. Use SPSS to determine if candy colourfulness after exposure to sunlight (colour_post) depends on candy weight and candy sweetness. Interpret the 95% confidence intervals for both effects. 
    ...
    Hint: Use regression analysis, which is available under *Analyze > Regression > Linear* in SPSS. [<img src="icons/2answer.png" width=115px align="right">](#answer3.6.4){.buttonToAnswer}
```
...
```{block2, type='rmdquestion', echo = Qch3}
1. What is the most likely estimate for average candy weight in the population? [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.1){.buttonToAnswer}
```

<A name="question3.7.2"></A>
```{block2, type='rmdquestion', echo = Qch3}
2. The percentage in between the two vertical lines can be interpreted as a probability. A probability of what? [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.2){.buttonToAnswer}
```

<A name="question3.7.3"></A>
```{block2, type='rmdquestion', echo = Qch3}
3. The double arrow represents an interval of sample means, in this example, average candy weight. What happens if you change the confidence level? Explain why this makes sense. [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.3){.buttonToAnswer}
```

<A name="question3.7.4"></A>
```{block2, type='rmdquestion', echo = Qch3}
4. What happens to the graph if you change sample size? [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.4){.buttonToAnswer}
```

<A name="question3.7.5"></A>
```{block2, type='rmdquestion', echo = Qch3}
5. What happens to the standard error if you change sample size? How are sample size and standard error linked? What characteristic of the sampling distribution is expressed by the standard error? [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.5){.buttonToAnswer}
```

<A name="question3.7.6"></A>
```{block2, type='rmdquestion', echo = Qch3}
6. The values of the interval limits---average candy weights in this example---on the scale in standard errors are called critical values. What happens to the critical values if you change sample size? [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.6){.buttonToAnswer}
```

<A name="question3.7.7"></A>
```{block2, type='rmdquestion', echo = Qch3}
7. What happens to the critical values if you change the confidence level? [<img src="icons/2answer.png" width=115px align="right">](#answer3.7.7){.buttonToAnswer}
```
