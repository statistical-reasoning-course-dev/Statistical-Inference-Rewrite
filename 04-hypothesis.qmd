...
\end{eqnarray*}
$$ {#eq-Hnull}
The null hypothesis does not always assume that the population parameter is zero; it can take any specified value. For instance, a null hypothesis might state that there is no difference in intelligence between communication science students and the general population. In this case, we could test whether the average intelligence score of a sample of communication science students is different from 100, the average intelligence score known from extensive prior work (population parameter = 100). Alternatively, we could test whether the difference between the intelligence scores of a sample of communication science students and a sample of the general population is 0 (population parameter = 0). These two research designs test the same hypothesis, but require different statistical tests, as you will learn in @sec-test-selection. ~~Although we are testing whether the difference is zero, in practical terms, we express the null hypothesis as the sample mean being equal to 100.~~ I SUGGEST TO REPLACE THIS SENTENCE WITH THE TWO SENTENCES I WROTE RIGHT BEFORE WHICH I THINK ARE MORE EXPLICIT. SEE WHAT YOU THINK. 

I THINK THIS PARAGRAPH COULD BE MORE EXPLICITLY CONNECTED TO THE REST OF THE CONCEPTS/CHAPTER. MAYBE SOMETHING LIKE: If we assume that the null hypothesis (a specific population parameter) is true (for instance, that the proportion of yellow candies is .5 in the population), we can make additional assumptions about what the population in question should look like and, thereby, create the so-called null distribution (a sampling distribution that reflects the true population if the the null hypothesis is correct). You have learned about three formal ways of creating such a distribution in I'M NOT SURE HOW TO REFERENCE THE CHAPTER, SORRY. For instance, as illustrated in @fig-nulldistribution, if the true proportion of yellow candies in the population is .5, randomly sampling a bag in which 1 out of 10 or 10 out of 10 candies is yellow is very unlikely, while sampling a bag with 4, 5, or 6 yellow candies are likely outcomes. ~~Though a null hypothesis can be expressed as a single value, that does not mean that we always get that specific value when we take a random sample. Given the null hypothesis that our candy factory machine produces bags with an average of 5 out of 10 yellow candies, there remains a probability that some bags will contain as few as one yellow candy or even none at all. This variability is illustrated in @fig-nulldistribution, which shows that while values near the expected 5 yellow candies are most likely, deviations can occur.~~
...
Lets revisit the case of our candy factory and assume that the machine produces candy bags in which, on average, 2 out of every 10 candies are yellow. In other words, the population parameter is .2. THE REST OF THIS PARAGRAPH IS ADJUSTED IN LINE WITH THE CHANGES I SUGGESTED FOR THE H0 SECTION ABOVE. Again, based on this alternative hypothesis, we can generate a probability distribution called the alternative disribution that describes the population accurately under the assumption that the alternative hypothesis (and not the null hypotheiss) it true (@fig-altdistribution). In this case, the most likely outcome of randomly sampling a candy bag from this population is finding 2 yellow candies in a bag, while finding 9 or 10 yellow candies is very unlikely. ~~Assuming .2 as the machine's parameter does not ensure that every bag will contain exactly 2 yellow candies. Some bags will contain 0, 1, 3, 4, 5, 6, 7, 8, 9, or even 10 yellow candies. The probabilities for each can again be visualized using the exact discrete binomial probability distribution (@fig-altdistribution) as we did for the null hypothesis.~~
...
\end{eqnarray*}
$$ {#eq-Halt}
~~The alternative hypothesis can be one-sided or two-sided. A two-sided alternative hypothesis states that the population value is not equal to the null hypothesis. A one-sided alternative hypothesis states that the population value is either greater than or less than the null hypothesis.~~ I SUGGESTED TO INTEGRATE THIS INFORMATION IN THE PARAGRAPH ABOVE The choice between a one-sided and two-sided hypothesis (and corresponding statistical test) depends on the research question and the theory. We will cover one and two-sided testing more extensively in @sec-one-twosidedtests.
...
I FIND THE BOX A LITTLE CONFUSING GIVEN THE EARLIER TEXT. THE REVISED TEXT NO LONGER SPECIFIES EXPLICITLY THAT THE NULL + ALTERNATIVE HYPOTHESES COVER ALL POSSIBLE VALUES (AND THEY DON'T IF YOU DON'T DISTINGUISH BETWEEN ONE- AND TWO-SIDED NULL HYPOTHESES.) WE ALSO GENERALLY DON'T COMPARE WHICH HYPOTHESIS FITS BETTER, BUT ONLY TEST WHETHER H0 IS PLAUSIBLE.
...
~~We will use both distributions in @fig-twobinom to clarify the different components of the Neyman-Pearson approach later in this chapter. For now, take a good look at both probability distributions, and consider a bag of candy containing 4 yellow candies. Are you able to determine if this bag is the result of a manufacturing process that produces bags with 20% or 50% yellow candies?~~ SIMILAR COMMENT AS BEFORE THAT THIS INTUTION OF COMPARING THE TWO DISTRIBUTIONS IS NOT FULLY CONSISTENT WITH THE TESTING APPROACH WE ACTUALLY IMPLEMENT. THE ONLY PLACE WE ACTUALLY CONSIDER HA IS WHEN WE TALK ABOUT POWER. I MOVED THE POINT ABOUT USING THE DISTRIBUTIONS TO DISCUSS THE APPROACH UP AND SUGGEST TO REMOVE THE REST. I THINK IT MIGHT BE A BIT CONFUSING FOR STUDENTS TO SEPARATE HA FROM HT. I ADDED A SENTENCE ABOUT IT ABOVE.

~~Doing research is essentially the same. You collect one sample, and have to determine if the effect of your study is non existent ($H_0 = \text{true}$) or that there is something going on ($H_0 \neq \text{true}$)~~ THIS INTUITION OF WHETHER THE EFFECT IS NIL OR NOT DOESN'T FIT THE CANDY EXAMPLE. CAN THIS BE TAKEN OUT?
...
Choosing such an alpha level would result in a decision threshold located between 0 and 1 yellow candies in the sample bag and 9 and 10 yellow candies in the sample bag. IN THE CONTINUOUS CASE, WE ALWAYS TALK ABOUT THE VALUE BEING AS EXTREME OR MORE EXTREME. TRANSLATING THIS HERE - SHOULDN'T THE CRITICAL VALUES TECHNICALLY BE 0 AND 10 RATHER THAN A NON-EXISTENT IN-BETWEEN NUMBER? We call this the **critical value** associated with the chosen alpha level. If the number of yellow candies in our sample bag is outside of this threshold (0 or 10 yellow candies) we would reject the null hypothesis. If the number of yellow candies in our sample bag is inside of the threshold (1, 2, 3, 4, 5, 6, 7, 8, or 9 yellow candies) we would not reject the null hypothesis. ~~So, if that is our decision criterion, we would reject the null hypothesis if we would draw a bag with 0 or 10 yellow candies, and not reject the null hypothesis if we would draw a bag with 1, 2, 3, 4, 5, 6, 7, 8, or 9 yellow candies. Amounting to a type I error rate of .002 or 0.2%.~~ @fig-nulldistributionalpha shows the critical values for the null hypothesis distribution, and indicate what the decision would be for values on the outside and inside of the decision boundary.
...
~~As we have already set our decision criterion by choosing our alpha level in the previous step, we already know when we decide to reject the null hypothesis. In @fig-nulldistributionalpha, based on the null distribution, we determined our type I error to be 0.2%, if we would reject the null hypothesis after drawing 0 or 10 yellow candies. The critical value would in that case be between 0 and 1 and 9 and 10. SEE COMMENT ABOVE We use this same critical value to determine the power of the test, as it establishes our decision boundary. ~~

~~As the right column of @fig-decisiontable only states that $H_0 = \text{FALSE}$, it does not state what this entails. Within the Neyman-Pearson approach, this would be the true population value with its associated probability distribution. We already established that this would be the distribution with a parameter value of .2. In @fig-altdistributionpower, we see that our decision criterion is still the same. That we decide to reject the null when we sample 0 or 10 yellow candies. But the distribution has now changed.~~  I FELT LIKE THESE PARAGRAPHS WERE A LITTLE DENSE/FULL OF TERMINOLOGY AND TRIED TO RE-WRITE THEM IN SOMEWHAT SIMPLER LANGUAGE. SEE WHAT YOU THINK.
...
Looking at @fig-nulldistributionalpha, we see that the critical value is between 0 and 1 and 9 and 10. HIGHLIGHTING FOR POTENTIAL CHANGE DEPENDING ON DECISIONS EARLIER. If we would draw a bag with 4 yellow candies, we can check if the value 4 is inside or outside the critical value. As 4 is inside the critical value, we would not reject the null hypothesis.
...
The p-value is based on the sampling distribution that was created during the test set-up. This sampling distribution gives the probability of randomly drawing a sample with different sample statistics from a population in which the null hypothesis is correct. ADDED THIS PREVIOUS SENTENCE. SEE YOU YOU THINK IT HELPS. Imagine we sampled a candy bag with 4 yellow candies. The p-value gives the probability of randomly drawing a sample that is as extreme or more extreme than our current sample assuming that the null hypothesis is true. “As extreme or more extreme” here means as far or further removed from the value specified by the null hypothesis. Concretely, in our case the null hypothesis predicted a population parameter of .5 (5/10 candies are yellow). That means the p-value indicates the probability of drawing a sample with 4 or fewer yellow candies. ~~The p-value considers the probability of such a sample, but also ads the probability of getting a sample with less yellow candies. This is what is meant with “at least as extreme”. this is not really intuitive, but it refers to the less likely test statistics, iIn our case 0, 1, 2 and 3, are even less probable than 4 yellow candies. ~~ THIS FELT A BIT REPETITIVE The assumption that the null hypothesis is true indicates that we need to look at the probabilities from the sampling distribution that is created based on the null distribution hypothesis. Looking at @tbl-nullprobtable, we see that the probability of drawing a random sample with 0, 1, 2, 3 or 4 yellow candies under the null distribution is 0.001 + 0.010 + 0.044 + 0.117 + 0.205 = 0.377 according to the sampling distribution belonging to the null hypothesis. This 0.377 is the p-value associated with our test statistic (4 yellow candies). In other words, the conditional probability (conditional on H0 being true) of randomly drawing a sample in which the test statistic is as far or further removed from H0 is .377. ADDED THE LAST SENTENCE TO FINISH THE THOUGHT.

> Rejecting the null hypothesis does not mean that this hypothesis is false or that the alternative hypothesis is true. Please, never forget this. I FEEL LIKE THIS COULD USE ANOTHER SENTENCE BELOW. MAYBE SOMETHING LIKE: IT IS IMPORTANT TO REMIND YOURSELF THAT EVERY DECISION YOU MAKE WITHIN THE Neyman-Pearson approach IS ASSOCIATED WITH A CERTAIN PROBABILITY OF MAKING A WRONG DECISION, EVEN IF THAT PROBABILITY IS TIGHTLY CONTROLLED. A TEST RESULT CAN LEND SUPPORT FOR OR EVIDENCE AGAINST A HYPOTHESIS, BUT CAN NEVER CONCLUSIVELY PROVE OR DISPROVE IT. WE DISCUSS LIMITATIONS OF NULL HYPOTHESIS SIGNIFICANCE TESTING IN MORE DETAIL IN SECTION...

The reasoning applied when comparing our test statistic to the critical value is the same as when comparing the p-value to the alpha level. If the p-value is equal to or smaller than the alpha level, we reject the null hypothesis. If the p-value is larger than the alpha level, we do not reject the null hypothesis. ISN'T IT MORE CONVENTIONAL TO LOOK FOR P<.05 (NOT P<=.05)?
...
Remember that the _p_-value is a probability derived from a sampling distribution, which can only be created based on concrete assumptions about the population. For instance, if our null hypothesis is that 20% of candies are yellow, we would assign a low probability to the event of randomly drawing a sample with 10 yellow candies. If our null hypothesis was that 80% of candies are yellow, we would assign different probabilities. ~~Compare it to the probability that we throw sixes with a dice. This probability is one out of six under the assumption that the dice is fair.~~ Probabilities rest on assumptions. If the assumptions are violated, we cannot calculate probabilities. I FEEL LIKE THIS POINT IS ALREADY MADE MORE CLEARLY/EXTENSIVELY IN THE CHAPTER ON SAMPLING DISTRIBUTIONS. MAYBE WE SHOULD REFER BACK TO THAT? I'M ALSO SUGGESTING TO STICK WITH THE SAME EXAMPLE HERE SO PEOPLE DON'T HAVE TO SWITCH CONTEXT MENTALLY.
...
The true effect size is the difference between the null hypothesis and the true population value. This can also be expressed in terms of the test statistic. For example, if the IQ scores for communication science students are 120 but our null hypothesis expected an IQ of 100 (equal to that of the general population), the true effect size can be expressed as 20 IQ points (unstandardized), but also as a  _t_-value (standardized). I ADDED STANDARDIZED AND UNSTANDARDIZED HERE. WAS THIS EXPLAINED ALREADY? The **true** effect size denotes the genuine effect within the population, representing the actual difference, correlation, or parameter value.

In the candy factory example, the true effect size is .5 (H0) - .2 (true population value) = .3. This is the difference in the proportion of yellow candies in the bags. In @fig-twobinomeffect you can see the difference in the two distributions. The true effect size is the difference in the expected value of the two distributions. In absolute terms, it is 5 - 3 yellow candies per bag. In terms of the parameter it is the proportion .5 - .2. DOES THIS LAST SENTENCE REFER OT THE IDEA THAT ONLY THE PROPORTION IS AN UNBIASED ESTIMATOR HERE? I FEEL LIKE THAT NUANCE WILL GO OVER PEOPLE'S HEADS. MAYBE REMOVE HERE? OR SAY EVERYTHING IN TERMS OF PROPORTIONS AND THEN END WITH - PRACTICALLY THAT MEANS 2 CANDIES DIFF...
...
Depending on the true value in the population, a true effect size could be small, medium, or large. To give you a rough intuition, Rains, Levine, and Weber (2018) summarized effect sizes in a a huge body of empirical work covering sixty years worth of communication science research across sub-fields involving over 8 million participants. Focusing on meta-analyses (a technique we cover in more detail in @sec-meta-analysis), they found that the mean effect estimate indicated weak associations between variables of interest (r = .21, see @sec-assoc-size for details on measures of association). In other words, when designing statistical tests for hypotheses about communication phenomena, we often have to account for the likelihood of small effect sizes. ADDED EVERYTHING AFTER THE FIRST SENTENCE. IF YOU FIND THIS STUDY USEFUL, PLEASE HELP ADDIGN THE REFERENCE: https://academic.oup.com/anncom/article/42/2/105/7905923

A statistical test needs to be designed in such a way that it is suitable to detect an effect size of interest. Imagine the statistical test you are setting up as a detection tool like a microscope. You are more likely to see things clearly if your microscopic specimen are larger or if your microscope has a stronger, more powerful lense. Similarly, statistical tests can generally detect larger effect sizes more easily (with more power). Small true effect sizes require more precision, which you can gain by increasing your sample size. ADDED THE MICROSCOPE ANALOGY HERE INSTEAD OF THE METAL DETECTOR BC IT SUPPORTS THE INTUITION ABOUT EFFECT SIZE AND PRECISION. SEE WHETHER YOU THINK ITS HELPFUL. ~~For example, we would reject the null hypothesis that average candy weight is 2.8 grams in the population if average weight in our sample bag is 2.70 grams and our sample is large. But we may not reject this null hypothesis if we have the same outcome in a small sample bag.~~ I FEEL LIKE THIS LAST SENTENCE IS STILL A BIT COMPLICATED WITHOUT HAVING EXPLAINED PRECISION. MAYBE JUST REFER TO THE SAMPLE SIZE SECTION FOR MORE DETAILS?
...
Note that the effect sizes depend on the scale on which we measure the sample outcome. The unstandardized effect size of average candy weight changes if we measure candy weight in grams, micro grams, kilograms, or ounces. Of course, changing the scale does not affect the meaning of the effect size but the number that we are looking at is very different: 0.05 grams, 50 milligrams, 0.00005 kilos, or 0.00176 ounces. For this reason, we do not have rules of thumb for interpreting these **unstandardized effect sizes** in terms of small, medium, or large effects. But we do have rules of thumb for **standardized effect sizes**. Unstandardized effect sizes are very useful for reporting the practical results of your study, but they are not very useful for comparing studies or for meta-analysis. I DON'T FULLY FOLLOW THIS COMMENT ON META-ANALYSIS. IS THE IDEA THAT DIFFERENT STUDIES USE SLIGHTLY DIFFERENT SCALES AND SAMPLES SO WE NEED TO STANDARDIZE TO COMPARE? I THINK THIS WILL GO OVER STUDENTS' HEAD. MAYBE MOVE THIS DOWN TO THE COHEN'S D-SECTION? I THINK THE ATTITUDE SCALE EXAMPLE IS MORE INTUITIVE. INSTEAD OF COHEN'S D THE SECTION COULD BE CALLED STANDARDIZED EFFECT SIZES AND NAME COHEN'S D AS AN EXAMPLE

Because the observed effect size is based on the result of drawing one random sample, its reliability is limited. ~~You can imagine that estimating the true effect size on just one sample is not very reliable. The observed effect size could be the result of our sample being the result of the null being true, or the alternative being true.~~ An important tool to get a more reliable estimate of the true effect size is replication. If the observed effect size is consistent over multiple replications, we can be more confident that the average observed effect size across studies is the true effect size. This is what we will cover in @sec-meta-analysis about meta analysis.
...
Using an inventory of published results of tests on one or two means, @RefWorks:3933 proposed rules of thumb for standardized effect sizes (ignore a negative sign if it occurs): THIS IS SPECIFIC TO COHEN'S D RIGHT? SINCE ANOTHER RULE OF THUMB IS GIVEN BELOW, I THINK THIS SHOULD BE MADE CLEAR HERE.
...
@fig-posthocpower shows the post hoc power for a sample of 10 candies. The null hypothesis is that the machine produces bags with 5 yellow candies. The alternative hypothesis is that the machine produces bags with 2 yellow candies. ISN'T THIS THE TRUE POPULATION PARAMETER - NOT JUST AN HA? But the post hoc power assumes the found test statistic of 4 candies to be the alternative population parameter of .4. Following the same decision criterion as defined in the previous sections, the post hoc power is almost zero. This is the probability of 0 or 10 yellow candies (the values that are as or more extreme than the critical values) under the true distribution I'M GETTING A BIT LOST HERE. SHOULDN'T THIS BE THE DISTRIBUTION BASED ON THE ASSUMPTION THAT THE OBSERVED .4 IS THE TRUE PROPORTION (WHICH IT ISN'T).

You can imagine that if we look at a different candy bag and we would find 7 yellow candies, the post hoc power would not be the same because the probabilities of drawing 0 or 10 yellow candies changes under the assumption of a popultion proportion of .7. The post hoc power does not have much practical use, though SPSS produces this when you ask it, it is obvious that multiple replications of a research study will yield different results. As the true population mean is not a random variable, the actual power is fixed. I'M WONDERING WHETHER VARIABILITY IS THE IMPORTANT POINT HERE. HOW ABOUT POINTING OUT THAT POST-HOC POWER DOES NOT HAVE PRACTICAL VALUE FOR THE CURRENT STUDY, BUT COULD BE INFORMATIVE IF YOU PLAN ON DESIGNING A REPLICATION STUDY THAT NEEDS TO BE PROPERLY POWERED.
...
As mentioned in @sec-observed-effect-size, the observed effect size is based on the sample statistic, and is likely to differ somewhat with every new sample you draw. ~~If our research hypothesis is actually true, a random sample from a population described by the sampling distribution of the alternative hypothesis would be more likely to result in us holding a bag with 2 yellow candies. But as we have seen in @fig-altdistribution, getting 4 yellow candies is reasonably probable as well.~~ LIKE IN THE SECTION ABOVE, THIS MIXES THE TRUE POPULATION PARAMETER WITH AN ALTERNATIVE HYPOTHESIS. I'M SUGGESTING TO JUST CUT IT. I THINK THE INTUITION IS REASONABLY CLEAR WITHOUT IT. 
...
~~Imagine that we get a hundred bags of candy (100 replications) and we consistently find 7 to 9 yellow candies, this would give us an indication that the true population value is 8. It would also indicate that our initial alternative hypothesis is highly unlikely.~~ SUGGESTING TO DELETE THIS, BECAUSE IT ALREADY GETS MUCH CLOSER TO GENERATING A REAL SAMPLING DISTRIBUTION BUT DOESN'T LINK TO PRACTICAL REALITY WHERE YOU MIGHT USE META-ANALYSIS. 
...
For instance, in 2015, Tannenbaum and colleagues published an influential meta-analysis in which they summarized the effects of fear appeals on attitudes, intentions, and behaviors found across 248 independent samples including a total of 27,372 people.Results confirmed a positive, weak to moderate effect of fear appeals (r=.29, see @sec-assoc-size). This result is substantially more reliable than any individual study and, what's more, the authors were able to identify moderators, situations under which fear appeals were more or less effective based on differences between the individual studies they included. ADDED THIS EXAMPLE. SEE WHAT YOU THINK. IF YOU LIKE IT, PLEASE HELP ADD THE CITATION Tannenbaum, M. B., Hepler, J., Zimmerman, R. S., Saul, L., Jacobs, S., Wilson, K., & Albarracín, D. (2015). Appealing to fear: A meta-analysis of fear appeal effectiveness and theories. Psychological Bulletin, 141(6), 1178–1204. https://doi.org/10.1037/a0039729
...
With this sample size, we can acquire our desired power of 80%. If we would assume our alternative hypothesis to be true, our decision to reject the null when you get 5 or less yellow candies, would be correct 80% of the time. The power of 80% is the sum of the light yellow bars on the distribution representing $H_A$, which represent values as or more extreme than the critical value. So, the power is the probability of getting 0, 1, 2, 3 ,4 ,5 or 15, 16,17, 18, 19, 20 yellow candies under the true SHOULDN'T THIS SAY ALTERNATIVE DISTRIBUTION? distribution.
...
The reasoning is again the same as in the discrete case, when we use categorical sample statistics. We first determine our desired alpha and power and make sure our sample size is large enough to get the desired power for our effect size of interest. Then, after collecting our data, we can calculate our test statistic and determine if we can reject the null hypothesis or not, being confident that we will be wrong in our conclusion in 5% of the cases when the null hypothesis is in fact true ADDED THE H0 IS TRUE QUALIFIER - PLEASE DOUBLE CHECK, and that we will be right in 80% of the cases when the alternative hypothesis is actually true.

THIS SECTION DEMONSTRATES THAT SAMPLE SIZE MAKES A DIFFERENCE, BUT I'M MISSING SOME OF TEXT FROM THE OLD BOOK VERSION THAT EXPLAINS WHY, WHICH WOULD REQUIRE A DEFINITION OF PRECISION AND MAYBE SHOWING HOW THE DISTRIBUTION BECOMES MORE OR LESS PEAKED DEPENDING ON SAMPLE SIZE. OR IS THAT CONTENT SOMEWHERE ELSE NOW?

#### How to determine sample size

As stated in @sec-power about the power of a test, we already considered that we do not know the parameter for the alternative distribution and that we therefore also don't know the true effect size. MAYBE I'M MISUNDERSTANDING THIS, BUT THIS IS AGAIN CONFOUNDING HA WITH HT, WHICH I THINK WAS PREVIOUSLY SEPARATED. NOT RE-WRITING IN CASE I'M MISSING SMTH HERE We stated that you can make an educated guess about the true effect size based on previous research, theory, or other empirical evidence. You can also consider which effect size would be practically relevant.
...
I FIND THIS A BIT DISRUPTIVELY PLACED. MAYBE MOVE DOWN AS A SORT OF SUMMARY/CONCLUSION?

::: {.callout-important appearance="simple" #rmdfisher}
...
1. @fig-null-bootstrap shows the bootstrapped sampling distribution of sample medians for media literacy of teenagers. Use the sliders to determine the 95% confidence interval of the sample median.
...
In null hypothesis significance testing, we totally rely on the test's _p_ value. If this value is below .05 SEE COMMENT ABOVE - TO BE CONSISTENT WITH THE REST OF THE TEXT IT SHOULD BE LESS OR EQUAL TO or another significance level we specified for our test, we reject the null hypothesis and we do not reject it otherwise. Based on this decision, we draw a conclusion about the effect in the population. Is this a wise thing to do? Watch @vid-video-cumming.
...
As the video illustrates, only focusing on the p-value can provide wildly misleading results. Specially with small sample sizes, you can find significant effects that do not correspond with results we find while using confidence intervals. Blindly following the p-value mantra is considered to be bad practice. I'M NOT SURE WHICH POINT IS BEING MADE HERE EXACTLY. IS IT JUST THAT THE CONCLUSIONS WE WOULD DRAW FROM P-VALUES ARE NOT ALWAYS THE SAME AS THE ONE'S WE WOULD MAKE BASED ON CONFIDENCE INTERVALS? IT NOW SOUNDS A BIT LIKE THAT THERE IS NO RELATION BETWEEN THE TWO WHICH MAYBE GOES A BIT FAR.

I hope that by now, @sec-null-hypothesis-significance-testing has prepared you to critically reflect on this video. In his simulation, Cumming correctly states that "studies have found that in many areas of Psychology, the median size effect is .5". Though blaming the _p_-value instead of questionable research practices is a bit misleading. We have learned that we should strive for a power of 80% and set our sample size accordingly. Looking at the overlap of the $H_0$ and $H_A$ distributions in the video it is clear to see that both distributions overlap. THERE IS A BIT OF A LOGICAL JUMP HERE BEFORE THE LAST SENTENCE. IS THE POINT TO SAY THAT THE EXAMPLES IN THE VIDEO ARE UNDERPOWERED?
