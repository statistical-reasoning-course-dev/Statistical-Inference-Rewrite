...
You might recognize that this example, and the video below, shows the example of candy colour and candy stickiness as described in @exact-intro. Thus, we still have two categorical variables which both have two categories. Hence, we have a 2x2 contingency table and perform an Exact test. Three ways are shown.

First we show you the approach via the _nonparametric tests > legacy dialogs > chi-square test_, where you only have to select the _Exact_ option and the time limit of five minutes. 
...
We will interpret the results in the next video.
...
As said, we will interpret the results of an Exact test in SPSS in the video below.
...
Check out the video for the step by step on how to perform an independent t-test with bootstrapping, more details and additional information.
