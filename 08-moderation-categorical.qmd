...
:::{#fig-concept-smoke}
```{r concept-smoke, echo=FALSE, fig.asp=0.4, fig.pos='H', fig.align='center'}
...
rm(variables)
```
A conceptual model with some hypothesized causes of attitude towards smoking.
:::
...
:::{#fig-moderator-concept2}
```{r moderator-concept2, echo=FALSE, fig.pos='H', fig.align='center', fig.asp=0.3}
# Create coordinates for the variable names.
...
The interaction variable must be included together with the original predictor and moderator variables, see Equation \@ref(eq:intvar). This is also visible in the statistical diagram (@fig-moderator-statistical) for moderation in a regression model.
...
:::{#fig-moderator-statistical}
```{r moderator-statistical, fig.pos='H', fig.align='center', echo=FALSE, fig.asp=0.4}
library(ggplot2)
# Create coordinates for the variable names.
...
ggplot(variables, aes(x, y)) + 
  ...
  theme_void()
```
Statistical diagram of moderation.
:::
...
:::{#fig-categorical-moderator-fig}
```{r categorical-moderator-fig, echo=FALSE, fig.pos='H', fig.align='center', fig.asp=0.4}
library(ggplot2)
# Create coordinates for the variable names.
...
ggplot(variables, aes(x, y)) + 
  ...
  theme_void()
```
Statistical diagram with a moderator consisting of three groups. Non-smokers are the reference group
:::
...
:::{#fig-moderator-covar-statistical}
```{r moderator-covar-statistical, fig.pos='H', fig.align='center', echo=FALSE, fig.asp=0.4}
library(ggplot2)
# Create coordinates for the variable names.
...
ggplot(variables, aes(x, y)) + 
  ...
  theme_void()
```
Statistical diagram of moderation with contact as covariate.
:::
...
:::{#fig-dich-moderator-cov-plot}
```{r dich-moderator-cov-plot, echo=FALSE, out.width="600px", fig.pos='H', fig.align='center'}
...
Creating categorical by numerical interaction predictors (2 methods).
:::
...
{{< video https://www.youtube.com/embed/9Yf2rHgoBt8
    width="100%"
    height="315" 
>}}

Estimating categorical by numerical moderation with regression in SPSS.
:::
...
Representing moderation by regression lines in a scatterplot in SPSS.
:::
