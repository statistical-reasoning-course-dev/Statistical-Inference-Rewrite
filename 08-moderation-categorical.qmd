...
Whether this predicted difference is small or large depends on the practical context. Is the predicted decrease in attitude towards smoking worth the effort of the campaign? In the example shown in Figure @fig-regression-continuous, one additional unit of exposure decreases the predicted attitude by 0.6. This seems to be quite a substantial change on a scale from -5 to 5.
...
If we want to apply a rule of thumb for the strength of the effect, we usually look at the standardized regression coefficient ($b^*$ according to APA, *Beta* in SPSS output). See Section @sec-assoc-size for some rules of thumb for effect size interpretation.
...
### Dichotomous predictors {#sec-dichpredictor}
...
The predictor variable *Exposure* is included in the interaction effect. As a consequence, the regression coefficient for this variable (green box in Figure @fig-regressiontable) expresses the effect of exposure on attitude towards smoking for the reference group on the other variable included in the interaction effect, namely, people who never smoked. A one unit increase in exposure predicts a 0.20 more negative (-0.197) attitude towards smoking **for people who never smoked**.
...
An interaction effect such as -0.27 for *Former smoker \* exposure* tells us the difference between the exposure effect for former smokers and the exposure effect for people who never smoked (reference group). A plot of the regression lines shows the different exposure effects (Figure @fig-regressionplot). The red line (effect of exposure for former smokers) has a stronger downward tendency than the blue line (exposure effect for people who never smoked). 
...
## Different Lines for Different Groups {#sec-categoricalmoderator}
...
In a conceptual diagram (Figure @fig-moderator-concept2), moderation is represented by an arrow pointing at another arrow. The moderator (smoking status) changes the relation between the predictor (campaign exposure) and the dependent variable (attitude towards smoking).
...
We used a similar diagram to express moderation in two-way analysis of variance (Section @sec-moderationanova). But now at least one of our independent variables is numeric, for example, the number of times the respondent has been exposed to the campaign.
...
In the current section, we discuss regression models with a numerical predictor and a categorical moderator. The next chapter (Chapter @sec-moderationcont) presents regression models in which both the predictor and the moderator are numerical.
...
# 1. In Figure @fig-interaction-var-effect, does the fat grey line represent the effect of exposure on attitude in a simple regression model, a multiple regression model, or both?
# 
...
The interaction variable must be included together with the original predictor and moderator variables, see Equation \@ref(eq:intvar). This is also visible in the statistical diagram (Figure @fig-moderator-statistical) for moderation in a regression model.
...
In Table @tab-dich-moderator-output, the effect of exposure on attitude depends on the value of smoking status because the model includes an interaction effect of exposure with smoking status (red). Non-smokers are the reference group on the smoking status dummy variable because they are coded 0. Therefore, the regression coefficient for exposure (blue) gives us the effect of exposure on smoking attitude **for non-smokers**. 
If you want to check this, plug in 0 for the smoking status variable in the regression equation that we may construct from Table @tab-dich-moderator-output. 
...
Interaction variables are used just like ordinary predictors, so the general assumptions of regression analysis apply. See Section @sec-regr-inference for a description of the assumptions and checks.
...
In the end, we have an interaction variable for all groups but one on the categorical moderator. Figure @fig-categorical-moderator-fig shows the statistical diagram. Estimation of the model yields point estimates (regression coefficients), confidence intervals, and *p* values for all independent variables (Table @tab-cat-moderator-results).
...
Interaction predictors show effect differences. In Table @tab-cat-moderator-results, the interaction predictors test the null hypotheses that the effect of exposure on attitude is equal for former smokers and non-smokers (*Exposure\*Former smoker*) or for smokers and non-smokers (*Exposure\*Smoker*) in the population.
...
It is recommended to check the number of observations per value of the moderator. For a categorical moderator, such as smoking status, a scatter plot of the dependent variable (vertical axis) by predictor (horizontal axis) with dots coloured according to the moderator category may do the job. The left panel of Figure @fig-common-support shows an example. Check that there are observations for more or less all values of the predictor in each color. If the scatter plot is hard to read, create a histogram of predictor values grouped by moderator categories, as in the right panel of Figure @fig-common-support.
...
Figure @fig-moderator-covar-statistical shows the statistical diagram of a moderation model with contact as covariate and Table @tab-dich-moderator-cov-output summarizes the estimated effects. How can we get rid of the moderator and covariate(s), so exposure is left as the only independent variable and we can plot regression lines for exposure effects?
...
As a first step, use the estimated values of the regression coefficients in the SPSS output (Table @tab-dich-moderator-cov-output) to create a regression equation [Eq. \@ref(eq:intvarcov1)]. Just start at the top of the table and write down the regression coefficients (*B*) and the independent variable names.
...
Repeat these steps but plug in the score 0 for the *smoker* predictor to obtain the simple regression line for non-smokers. Figure @fig-dich-moderator-cov-plot shows the two regression lines and their equations. The effect of exposure on attitude is more strongly negative for smokers than for non-smokers.
...
## A Dichotomous or Categorical Moderator in SPSS {#sec-catmodSPSS}
