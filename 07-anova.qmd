...
#| fig-cap: "A means plot showing that average willingness to donate is higher with a celebrity endorser than without a celebrity endorser (Error bars = 95% Confidence Intervals). As a reading instruction, effects of endorsers are represented by arrows and dashed lines."
#| echo: false
#| warning: false

source('data/simulate_donors.R')
sim_data=simulate_data(Nobody_avg=3,Clooney_avg=6,Jolie_avg=7)
...
d <- sim_data %>%
  group_by(endorser) %>%
  summarise(
    willingness_av = mean(willingness),
    se = sd(willingness) / sqrt(n()),
    lower = willingness_av - qt(0.975, df = n()-1) * se,
    upper = willingness_av + qt(0.975, df = n()-1) * se,
    .groups = "drop"
  ) %>%
  mutate(
    const = "line_group",  # for line connection
    endorser=factor(endorser, levels=c('Nobody','Clooney','Jolie'))
  )

baseline <- d$willingness_av[d$endorser == "Nobody"]
clooney <- d$willingness_av[d$endorser == "Clooney"] 
jolie <- d$willingness_av[d$endorser == "Jolie"] 
ggplot(data.frame(d), aes(endorser, willingness_av)) + 
  ...
  geom_segment(aes(x = 2.1, xend = 2.1, y = baseline, yend = clooney),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
   geom_segment(aes(x = 1.7, xend = 2.3, y = clooney, yend = clooney),
               linetype = "dashed", color = "darkgrey") +
  geom_label(aes(x = 1.8, y = (baseline + clooney)/2,
            label = "Clooney effect",
            hjust = 0), color = "darkgrey",fill='white'
            ) +
  geom_segment(aes(x = 3.1, xend = 3.1, y = baseline, yend = jolie),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
  geom_segment(aes(x = 2.7, xend = 3.2, y = jolie, yend = jolie),
               linetype = "dashed", color = "darkgrey") +
  geom_label(aes(x = 2.8, y = (baseline + jolie)/2,
            label = "Jolie effect",
            hjust = 0), color = "darkgrey",fill='white'
            ) +
  theme_general() + 
...
#| fig-cap: "Means plots for the main effects of endorser and sex on willingness to donate (Error bars = 95% Confidence Intervals). As a reading instruction, effects of endorsers and of being female are represented by arrows and dashed lines."
#| echo: false
#| warning: false

sim_data <- simulate_data(ClooneyFemale_avg=6.5,ClooneyMale_avg=5,
                   JolieFemale_avg=8.5,JolieMale_avg=7,
                   NobodyFemale_avg=4.5,NobodyMale_avg=3)
...
ggplot(d, aes(endorser, willingness_av)) + 
    geom_point(size = 3, color=brewercolors["Blue"]) + 
    geom_line(aes(group = const), size = 1, color=brewercolors["Blue"]) + 
    geom_segment(aes(x = 1, xend = 3.1, y = willingness_av[[1]], yend = willingness_av[[1]]),
               linetype = "dashed", color = "darkgrey") +
    geom_segment(aes(x = 2.1, xend = 2.1, y = willingness_av[[1]], yend = (willingness_av[[2]] - 0.1)),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
  geom_segment(aes(x = 1.7, xend = 2.3, y = willingness_av[[2]], yend = willingness_av[[2]]),
               linetype = "dashed", color = "darkgrey")+
    geom_label(aes(x = 1.8, y = (willingness_av[[1]] + willingness_av[[2]])/2,
            label = "Clooney effect",
            hjust = 0), color = "darkgrey",fill='white'
            ) +
    geom_segment(aes(x = 3.1, xend = 3.1, y = willingness_av[[1]], yend = (willingness_av[[3]] - 0.1)),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
  geom_segment(aes(x = 2.7, xend = 3.3, y = willingness_av[[3]], yend = willingness_av[[3]]),
               linetype = "dashed", color = "darkgrey")+
    geom_label(aes(x = 2.8, y = (willingness_av[[1]] + willingness_av[[3]])/2,
            label = "Jolie effect",
            hjust = 0), color = "darkgrey",fill='white'
            ) +
    theme_general() + 
...
d=sim_data %>% group_by(sex) %>%
  summarise(willingness_av = mean(willingness),
            se = sd(willingness) / sqrt(n()),
            lower = willingness_av - qt(0.975, df = n()-1) * se,
            upper = willingness_av + qt(0.975, df = n()-1) * se,
            .groups = "drop") %>%
...
a=ggplot(d,aes(sex, willingness_av)) + 
    geom_point(size = 3, color=brewercolors["Blue"]) + 
    geom_line(aes(group = const), size = 1, color=brewercolors["Blue"]) + 
    geom_segment(aes(x = 1, xend = 2.1, y = willingness_av[[1]], yend = willingness_av[[1]]),
               linetype = "dashed", color = "darkgrey") +
  geom_segment(aes(x = 1.8, xend = 2.2, y = willingness_av[[2]], yend = willingness_av[[2]]),
               linetype = "dashed", color = "darkgrey") +
    geom_segment(aes(x = 2.1, xend = 2.1, y = willingness_av[[1]], yend = (willingness_av[[2]] - 0.1)),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
    geom_label(aes(x = 1.7, y = (willingness_av[[1]] + willingness_av[[2]])/2-0.2,
...
sim_data<-simulate_data(NobodyMale_avg=3,NobodyFemale_avg=4.5,ClooneyMale_avg=5,ClooneyFemale_avg=9,JolieMale_avg=5,JolieFemale_avg=6.5)

library(ggplot2)

d=sim_data %>% group_by(endorser,sex) %>%
  summarise(
    willingness_av = mean(willingness),
    se = sd(willingness) / sqrt(n()),
    lower = willingness_av - qt(0.975, df = n()-1) * se,
  ...
  mutate(
    const = "line_group",  # for line connection
    endorser=factor(endorser, levels=c('Clooney','Nobody','Jolie'))
  ) %>% arrange(endorser)

ggplot(d, aes(endorser, willingness_av, colour = sex)) + 
  geom_point(size = 3) + 
  geom_line(aes(group = sex), size = 1) + 
  geom_segment(aes(x = 0.8, xend = 2.2, y = willingness_av[[1]], yend = willingness_av[[1]]),
               linetype = "dashed", color = "darkgrey") +
  geom_segment(aes(x = 2, xend = 2, y = willingness_av[[3]], yend = willingness_av[[1]]-0.1),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
  geom_label(aes(x = 1.82, y = (willingness_av[[1]] + willingness_av[[3]])/2,
            label = "Clooney effect\nfor females",
            hjust = 0), color = "darkgrey",fill='white'
            ) +
  geom_segment(aes(x = 0.8, xend = 2.2, y =  willingness_av[[4]], yend =  willingness_av[[4]]),
               linetype = "dashed", color = "darkgrey") +
  geom_segment(aes(x = 0.8, xend = 1.2, y =  willingness_av[[2]], yend =  willingness_av[[2]]),
               linetype = "dashed", color = "darkgrey") +
  geom_segment(aes(x = 0.9, xend = 0.9, y = willingness_av[[4]], yend = (willingness_av[[2]] - 0.1)),
               color = "darkgrey",
               arrow = arrow(length = unit(2,"mm"),
                                 # ends = "both",
                                 type = "closed")) +
  geom_label(aes(x = 0.8, y = (willingness_av[[2]]+willingness_av[[4]])/2-0.2,
            label = "Clooney effect\nfor males",
            hjust = 0), color = "darkgrey",fill='white'
            ) +
  theme_general() + 
