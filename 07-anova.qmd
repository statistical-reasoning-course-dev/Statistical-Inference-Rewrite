...
language used) does *not* matter to vaccine acceptance.
...
Download: [vaccine.sav](data/vaccine.sav) to follow along with the SPSS videos.
...
#| fig-cap: "Means plots for the main effects of language condition and health literacy on vaccine acceptance (Error bars = 95% Confidence Intervals). As a reading instruction, effects of language conditions and having high health literacy are represented by arrows and dashed lines."
#| echo: false
#| warning: false
...
literacy. We have six subgroups with average acceptance scores that we
have to compare.
...
3.  If you want to interpret an interaction effect, create means plots with confidence intervals, 
    ...
    larger, with non overlapping confidence intervals, for autonomy supportive language (pink distance in the middle)
    than for neutral language (pink distance on the left), and it is smallest
    for controlling language (pink distance on the right). Similarly, we see
    ...
    literacy (right-hand panel, pink distance on the right) than for people
    with low health literacy (right-hand panel, pink distance on the left).

::: {#fig-ANOVAplots .lightbox}
