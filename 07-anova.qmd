...
::: {#fig-anova-meansplot}
```{r anova-meansplot, echo=FALSE, fig.pos='H', fig.align='center', fig.asp=0.6, out.width="70%"}
...
rm(d)
```
A means plot showing that average willingness to donate is higher with a celebrity endorser than without a celebrity endorser. As a reading instruction, effects of endorsers are represented by arrows.
:::
...
1. In @fig-anova-within, samples are drawn from three populations that have the same means and variances. Do the samples have the same means?

2. How does between-groups variance in the samples relate to the variance in the populations?

3. What happens if you set population variance to zero in @fig-anova-within?

4. What, do you expect, is within-groups variance in @fig-anova-within?
...
1. In which situation is between-groups variance zero? Adjust the group means in @fig-anova-F to check your answer.
```
...
1. How should you adjust between-groups variance and within-groups variance to get more convincing differences between the average group scores in @fig-anova-Fratio?
...
```
SPSS means plots of the interaction effect of sex and endorser on willingness to donate. Note that the pink boxes have been added manually to aid the interpretation.
:::
...
```{=html}
<iframe src="https://sharon-klinkenberg.shinyapps.io/anova-twoway/" width="100%" height="408px" style="border:none;">
</iframe>
``` 
...
rm(df)
```
Number of observations per subgroup in a balanced 3x2 factorial design.
:::
...
:::{#fig-anova-diagram}
```{r anova-diagram, echo=FALSE, fig.pos='H', fig.align='center',  fig.asp=0.3}
library(ggplot2)
...
:::{#fig-anova-effectdirmod1}
```{r anova-effectdirmod1, echo=FALSE, out.width="70%", fig.asp=0.6, fig.pos='H', fig.align='center'}
# Add means plot with Clooney, Jolie, and Nobody on the x axis, willingness to donate on the y axis, and separate (coloured) lines linking the mean scores for females and males. In the plot, the average for females*Clooney is much higher than males*Clooney but the reverse does not apply to Jolie.
...
rm(d)
```
Moderation as a positive  effect in one context and a negative effect in another context.
:::
...
:::{#fig-anova-effectdirmod2}
```{r anova-effectdirmod2, echo=FALSE, out.width="70%", fig.asp=0.6, fig.pos='H', fig.align='center'}
# Add means plot with Clooney, Jolie, and Nobody on the x axis, willingness to donate on the y axis, and separate (coloured) lines linking the mean scores for females and males. In the plot, the average for females*Clooney is much higher than males*Clooney but the reverse does not apply to Jolie.
...
rm(d)
```
Moderation as opposite effects in different contexts.
:::
