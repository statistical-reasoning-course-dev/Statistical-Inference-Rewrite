...
In the video below we show you how to perform a single or parallel mediation model with PROCESS. You must have PROCESS installed to follow along.
...
For the interpretation of all the other associations and more details, please watch the video below.
...
For more details and a step-by-step instruction, please watch the video below.
...
In the next video we show you how to run a mediation with a covariate. The political communication team now studies the hypothesis that age on reading time is mediated by news site use, while controlling for education and political interest. We start the exact same way as before, `Analyze > Regression > PROCESS v4.1`. We select model 4 as we have a single mediator mediation model. All proceeds like the parallel mediation we showed before, but now we add the variables _education level_ and _interest in politics_ as `covariate(s)`.
...
The output works the same as the two times before, we only have additional independent variables in our output tables because of the covariates. Please watch the video below for a more in depth and detailed discussion of the output.
...
Mediation and moderation (Chapters @sec-moderationcat and @sec-moderationcont) can occur in the same model. For example, the effect of age on newspaper reading time mediated by interest in politics can be different for females and males. In other words, the indirect effect is different for females and males.
