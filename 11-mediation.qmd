...
Watch this micro lecture on mediation for an overview of the chapter (@vid-intro-mediation).

::: {#vid-intro-mediation}

{{< video https://www.youtube.com/embed/_cKP8d3ROtM
    width="100%"
    height="360" 
>}}
...
:::{#fig-mediationtable}
```{r mediationtable, echo=FALSE, out.width="75%", fig.pos='H', fig.align='center'}
knitr::include_graphics("figures/S11_AE1.png")
```
PROCESS output with estimated indirect and total effects of age on average newspaper reading time.
:::

@fig-mediationtable shows the results of a model in which political interest and news site use mediate the effect of age on newspaper reading time. Apart from a direct effect (_b_ = 4.55, 95%CI[3.62, 5.47], red box in @fig-mediationtable), three indirect effects have been estimated. Clearly the strongest indirect effect of age is the effect mediated by news site use (@fig-mediationtable, green boxes). An additional unit of age increases predicted newspaper reading time by 1.44. The 95% confidence interval (95%CI[0.64, 2.24], bootstrapped) does not include zero, so this indirect effect is statistically significant at the .05 significance level.

## Mediation as Causal Process

::: {#fig-mediation-commoncause .column-page-inset-right}

```{=html}
<iframe src="https://sharon-klinkenberg.shinyapps.io/mediation-commoncause/" width="100%" height="325px" style="border:none;">
</iframe>
``` 
How does a common cause affect regression coefficients? The values in this path diagram represent standardized regression coefficients.
:::
...
A third criterion for causality is that the correlation is not spurious. In @sec-spuriousness, we have encountered a spurious effect as an effect that incorrectly includes the effect of a confounder (reinforcer).
...
If we include both age and interest in politics as predictors in a regression model for newspaper reading time, the partial effect (@sec-partialeffect) of interest in politics is corrected for the spurious correlation between interest in politics and newspaper reading caused by age as their common cause. The partial effect of political interest can be interpreted as causal if current interest in politics was attained before the newspaper readings that we measure (very plausible) and age is the only common cause of interest in politics and newspaper reading (highly questionable).


```{r}
#| label: fig-causaldiagram
#| fig-cap: "Causal diagram for the effects of age and interest in politics on newspaper reading time."
#| echo: false
#| warning: false
...
Now let us draw the _causal diagram_ for this simple example (@fig-causaldiagram). A causal diagram contains the names of the variables with arrows pointing from causes to consequences. The causal order of variables is represented from left to right. In @fig-causaldiagram, the very first cause (age) is at the left, the final consequence (newspaper reading time) is at the right, and interest in politics is placed in the middle. In this layout, the arrows always point to the right.
...
An indirect effect may contain more than one step or mediator. If we include news site use in the model (@fig-causaldiagram2), we would have an indirect effect of age via interest in politics via news site use on newspaper reading time.
...
```{r}
#| label: fig-mediation-regression
#| fig-cap: "Path diagram with unstandardized effect sizes and their 95% confidence intervals."
#| echo: false
#| warning: false
...
visNetwork(nodes, edges, width = "100%") %>% 
  ...
  visInteraction(dragNodes = FALSE, multiselect = FALSE)
```


In the path diagram displayed in @fig-mediation-regression, we would regress newspaper reading time, the final dependent variable, on age and political interest. As a next step, we would predict political interest as outcome from age. 
...
4. All regression models meet the assumptions for regression analysis. Check if the residuals are normally distributed, centered around zero for all levels of the predicted outcome scores, and that scores are predicted equally well at all outcome levels (see @sec-regr-inference).

### Size of indirect effects {#sec-size-indirect-effects}
...
The size of an indirect effect is calculated in exactly the same way as the size of indirect correlations (@sec-indirectcorrelation): Just multiply the size of direct effects. This can be done with either the standardized regression coefficients or the unstandardized regression coefficients.

```{r}
#| label: fig-pathdiagram-unstandardized
#| fig-cap: "Path diagram with unstandardized effect sizes and their 95% confidence intervals."
#| echo: false
#| warning: false
#| eval: false
...
If each indirect effect in a path model contains at most one mediator, we have _single mediation_ or _parallel mediation_. @fig-mediationparallel illustrates single and parallel mediation. 

```{r}
#| label: fig-mediationparallel
#| fig-cap: "Causal diagrams for:."
#| fig-subcap: 
#|   - "Single mediation."
#|   - "Parallel mediation."
#| echo: false
...
If at least one of the indirect effects in a path model contains two or more mediators, we are dealing with _serial mediation_. @fig-mediationserial illustrates serial mediation. It contains an indirect effect from age on reading time with two mediators: Age > Political Interest > News Site Use > Reading Time. The distinction between parallel and serial mediation is relevant to the software (PROCESS) that we will use to estimate indirect and total effects (@sec-SPSSPROCESS).

```{r}
#| label: fig-mediationserial
#| fig-cap: "Causal diagram for serial mediation."
#| echo: false
#| warning: false
...
We use bootstrapping to create the sampling distribution of the size of an indirect effect. We have learned the principles and limitations of bootstrapping in @sec-boot-approx, so we do not have to go into details here. Suffice it to repeat that our original sample must not be too small and it must be quite representative of the population if we apply bootstrapping.

```{r}
#| label: tbl-indirect-sim
#| fig-cap: "Bootstrap results for unstandardized indirect effects in a model with two mediators. Effect size, standard error, lower and upper levels of the 95\\% confidence interval."
#| echo: false
#| warning: false
...
knitr::kable(results, booktabs = TRUE,
             col.names = c("", "Effect", "Boot SE", "BootLLCI", "BootULCI"), align = c("l", "r", "r", "r", "r"))  %>%
...
variables <- data.frame(x = c(0.3, 0.5, 0.7), 
                        ...
                        label = c("Age", "Pol.Interest", "Reading Time"))
ggplot(variables, aes(x, y)) + 
annotate("segment", x = 0.3,  y = 0.1, xend = 0.64, yend = 0.1,  arrow = arrow(length = unit(0.04, "npc"), type = "closed")) +
annotate("segment", x = 0.3,  y = 0.1, xend = 0.46, yend = 0.26, arrow = arrow(length = unit(0.04, "npc"), type = "closed")) +
annotate("segment", x = 0.5,  y = 0.3, xend = 0.66, yend = 0.14, arrow = arrow(length = unit(0.04, "npc"), type = "closed")) +
  geom_label(aes(label = label)) + 
  geom_text(x = c(0.35, 0.5, 0.65), 
            y = c(0.26, 0.07, 0.26), 
            label = c(paste("b", " = 0.30,\n95% CI [0.22, 0.38]"),
                      paste("Indirect: ", "b", " = 0.06, 95% CI [-0.02, 0.14]"), 
                      paste("b", " = 0.20,\n95% CI [0.12, 0.28]"))
            ) +
  coord_cartesian(xlim = c(0.2, 0.8), ylim = c(0, 0.4)) +
  theme_void()
...
```{r}
#| label: fig-causaldiagram4
#| fig-cap: "Causal diagram for  interest in politics as mediator between age and newspaper reading time with education as covariate."
#| echo: false
#| warning: false
...
@fig-causaldiagram4 presents a model in which education is used as a covariate in a model with political interest mediating the effect of age on newspaper reading time. Education is probably causally prior to both political interest and newspaper reading time, so it is allowed to have an effect on both variables. In this way, we control for education and remove spurious correlation between political interest and newspaper reading time due to education as a common cause.

If education predicts political interest and political interest predicts newspaper reading time as in @fig-causaldiagram4, political interest mediates the effect of education on newspaper reading time. We are, however, not interested in mediation in the case of a covariate, so we do not estimate or report the indirect effects of education on newspaper reading time. In the context of a mediation model, a _covariate_ is a predictor for which we do not investigate if its effect is mediated.
...
We analyse a path model as a series of regression models, so the general rules for reporting mediation are the same as for reporting regression analyses (see @sec-reportmoderation). If you summarize results in a table, make sure that the table includes:
...
#| fig-cap: "Unstandardized effects in a model regressing newspaper reading time on age with one mediator (News Site Use) and two covariates (Education, Political Interest). Theoretical approximation for direct effects, bootstrap results for indirect effects, using 5,000 bootstraps."
#| echo: false
#| warning: false
...
knitr::kable(results, booktabs = TRUE,
             col.names = c("", "B", "", "95% CI"), align = c("l", "r", "l", "c")) %>%
...
A path model may yield a lot of direct effects, so it is good practice to present results as a path diagram with the values of the standardized or unstandardized regression coefficients as labels to the arrows. A path model conveniently summarizes the results for the reader (@fig-results-pathdiagram). Remember that we don't use standardized regression coefficients if the predictor or a covariate is dichotomous variable or a set of dummy variables (see @sec-dichpredictor).


```{r}
#| label: fig-results-pathdiagram
#| fig-cap: "Unstandardized direct effects for a path model with one mediator and two covariates."
#| echo: false
#| warning: false
...
Inform the reader that you bootstrapped the indirect effect and report the number of bootstrap samples and the method used for the confidence intervals (see @sec-SPSSPROCESS). For a more elaborate discussion of reporting mediation, see Hayes [-@RefWorks:3873: 198-202].
...
Instructions on how to add PROCESS to your SPSS installation can be found in [Appendix -@sec-installPROCESS]. You download the PROCESS macro and install the SPSS custom dialog file. Check the FAQ at the PROCESS website if installation is not successful. If PROCESS is successfully installed, it can be found in the `Analyze > Regression menu`.

----

In @vid-SPSSmediatpar we show you how to perform a single or parallel mediation model with PROCESS. You must have PROCESS installed to follow along.

A political communication team wants to know which factors could have an impact on people reading the news paper, especially during election time. The team hypothesizes a mediation model with the question whether age (variable: _age_) is related to newspaper reading time (variable: _readingtime_), and whether this association can be explained by one's policital interest (variable: _polinterest_) and/or one's news site use (variable: _newssite_). We can model this with a mediation model, you can find PROCESS under `Analyze > Regresion > PROCESS`. 

We need to tell PROCESS that it needs to build a parallel mediation model, we can do so by selecting model number 4 under the `Model number:` dropdown menu. The `Confidence intervals` are set on 95 as default, in most cases that will be the ideal setting. The default `Number of bootstrap samples` being 5000 will also be the used setting in most cases. When implementing the variables, the window is categorized as a conceptual model X > M > Y. The dependent variable is the `Y variable`, in this case: average newspaper reading time. The independet variable is the `X variable`, in this case: age. The mediators, political interest and news site use, can be dragged to the `Mediator(s) M` box. For any additional settings we need to open the `Options` window. Please select `Show total effect model` and `standardized effects`. You can put the `Decimal places in output` to 3 to automatically help you with rounding. The other options are fine as the default option. Before running your analysis, please select `Long variable names` and click `I accept the risk of incorrect output`. This means that a variable like _polinterest_ only shows the first 8 characters, namely _polinter_. This is only a problem if you have two variables that have the exact same first eight characters. That rarely happens, but it is still good to know.

You can now run the analysis, we do not use paste with PROCESS as the syntax is unusable. It could be that PROCESS takes a bit longer to run than normally, no need to stress about that. Since you do not use paste for PROCESS, you should include a comment in your syntax to explain the model you have executed. In this case you could write `*PROCESS mediation, Model 4, Y = readingtime, X = age, M = polinterest, newssite.` The * opens the note and a dot closes the note in your syntax.

Let's take a look at the output. As with our other output, the output starts with a small summary: which variables were included and your sample size. Here you can check your model and whether you did the input (X, Y and M) correctly. Then we see different sets of output which all look like regression tables. Per tabel it is specified which varaible was the outcome variable. This tells you which lines of the regression model can be found here. For instance, the first table shows the output for the outcome variable _polinter_, only age is a independent variable here. This means that this table gives us the statistical results for the association between age and political interest (age > polinterest). 

_For anyone who wants a more in depth explanation of outcome variables and these different regression tables:_ If you want to figure out which line the output is providing information on, the outcome variable is the variable that receives the arrow in your model (thus the pointy end of the arrow points at the outcome variable). You will see several output tables but never with outcome variable age, because age is identified as the independent variable PROCESS knows that there are now arrows pointing towards age and thus age never needs to be an outcome variable. This is why identifying the variables correctly is very important, but also the model number. As PROCESS knows how to create your model based on the model number (for instance, in table two PROCESS knows that political interest does not need to be included in the output table with outcome variable news site use, because PROCESS knows we are doing a parallel mediation and not a serial mediation).

The interpretation of the output table works like a regression. The first column provides the coefficient, for age on political interest this is .099. Later in the table we see both the p-value, .042, and the 95%-confidence interval, [.004; 1.95]. The p-value is below .05 and the confidence interval does not include the zero, meaning this association is statistically significant.

All the way at the bottom of the output, you can find the output for your total effect, direct effect and indirect effect (the option we selected earlier, show total effects). As we have learned, the total effect consists of the direct effect (X on Y, i.e., age on reading time) and the total indirect effect (in this case two; via political interest and via news site use). You can see that summing up the direct and total indirect effect leads to the exact number of the total effect. We see that all effect are statistically significant, and the confidence intervals suggest that all effects are positive effects in the population. In the standardized indirect effects table we can compare the effects and see that news site use has a stronger effect than political interest.

_Note_ we do not use standardized effects for a dichotomous variable, but just the unstandardized.

For the interpretation of all the other associations and more details, please watch the @vid-SPSSmediatpar below.

::: {#vid-SPSSmediatpar}

{{< video https://www.youtube.com/embed/YLxODUTflbQ
    width="100%"
    height="315" 
>}}

Estimating a single or parallel mediation model with PROCESS.
:::
...
Now we move on to executing a serial mediation. The political communication team now wants to test a different theory, what if the mediators are working in a certain order? What if the association between age and news paper reading time are explained by political interest via news site use? we have to create a serial mediation in PROCESS, starting with `Analyze > Regression > PROCESS`.

To estimate a serial mediation, we use model 6 under the `Model number` dropdown menu. The `Confidence intervals` and `Number of bootstrap samples` keep their default setting of 95 and 5000. We select reading time as our outcome variable (Y), age as our independent variable (X) and political interest and news site use as our mediators (M). Since we model a serial mediation now, the order of the mediators is important. The order should correspond with your mediation model. In this case, first political interest, then news site use. Under `Options` we only select `Standardized effects` and you can opt for 3 `Decimal places in output`. Don't forget the `Long variable names`. Then, run your analysis, do not paste (the syntax is unusable). Do not forget to put a comment in your synyax file.

Let's take a look at the output. In the summary we can again check the input of our variables and the order of our mediators (M1, M2, ...). Like the previous mediation, we get several different output tables with different outcome variables. Check the outcome variable well before interpreting the results of the associations. Since SPSS now knows you are performing a serial mediation, political interest shows up as an independent variable for news site use (and it did not in the previous, parallel, mediation).

At the bottom we can see the direct and indirect effect again (notice there is no total effect as we did not ask for that this time). There are multiple indirect effect, but the pathway of the indirect effect is also shown at the bottom (e.g., ind1 age > polinter > readingt).

For more details and a step-by-step instruction, please watch the @vid-SPSSmediatserial below.

::: {#vid-SPSSmediatserial}

{{< video https://www.youtube.com/embed/VN_mgu5CQ4Y
    width="100%"
    height="315" 
>}}

Estimating a serial mediation model with PROCESS (Model 6).
:::
...
In the next video we show you how to run a mediation with a covariate. The political communication team now studies the hypothesis that age on reading time is mediated by news site use, while controlling for education and political interest. We start the exact same way as before, `Analyze > Regression > PROCESS`. We select model 4 as we have a single mediator mediation model. All proceeds like the parallel mediation we showed before, but now we add the variables _education level_ and _interest in politics_ as `covariate(s)`.

Under options we now select `Effect size` and opt for 3 decimal places again. Do not forget the long variable names and run your analysis (do not paste). Do not forget to place a comment in your syntax file and then take a look at the output.

The output works the same as the two times before, we only have additional independent variables in our output tables because of the covariates. Please watch @vid-SPSSmediatcov below for a more in depth and detailed discussion of the output.

::: {#vid-SPSSmediatcov}

{{< video https://www.youtube.com/embed/qK1KYgSwpss
    width="100%"
    height="315" 
>}}

Estimating a (single mediator) mediation model including covariates with PROCESS.
:::
...
The last video we will show you in this chapter (and this book all together), is how to estimate a path model in SPSS and how to check your assumptions for a mediation analysis.

For the last video, the political communication team is looking at the biggest model yet. We have a parallel mediation including a serial mediation. Because of this complicated structure, we cannot estimate this model in PROCESS. For now, that is perfect as you can also not check the assumptions in PROCESS. So, we are killing two birds with one stone here by showing you 1. how to proceed with a complicated mediation model like this one and 2. how to check your assumptions (also in the case of the mediations above).

Do we remember what we talked about earlier with the outcome variable being the variable receiving the arrow? Or the pointy end of the arrow points toward the outcome variable of an output table in PROCESS? We use that same principle this time to decide how many regression analyses we should run. Every variable that _receives_ an arrow, gets its own regression analysis. In this case, we need four: for education, political interest, reading time and news site use.

In the video we start with the regression model for news paper reading time, so that will be the regression we will discuss here as well. Go through `Analyze > Regression > linear`. Readingtime will be our `Dependent` variable, and the other variables (on the other side of the arrow) are all `Independent(s)`, in this case age, education, polinterest and newssite. Under the `Statistics...` window we select `confidence intervals` in addition to the default settings (estimate and model fit). Under `Plots...` we need the *ZPRED on the X-axis and the *ZRESID on the Y-axis to check our assumptions (you might remember this from our regression assumptions), and we select `Histogram`. Then you paste (as we are not using PROCESS anymore) and then run from your syntax.

In the output we can check the histogram for the assumption of normally distributed residuals, this one seems to be met. In the scatterplot we can check the assumptions of linearity and homoscedascity, which also seem to both be met. All these assumptions should sound familiar as we are checking the assumptions belonging to a regression analysis. Mediation is just combined regression analysis. 

For the step by step regression and a repeat of the assumptions (and visuals), please watch the @vid-SPSSpath.

::: {#vid-SPSSpath}

{{< video https://www.youtube.com/embed/olOEqeZE9WE
    width="100%"
    height="315" 
>}}

Getting standardized direct effects and checking assumptions in SPSS,
:::
...
Mediation and moderation (@sec-moderationcat and @sec-moderationcont) can occur in the same model. For example, the effect of age on newspaper reading time mediated by interest in politics can be different for females and males. In other words, the indirect effect is different for females and males.

If the indirect effect is different for females and males, at least one of the two direct effects (predictor on mediator or mediator on dependent variable) must be different for females and males. In @fig-causaldiagram5, the direct effect of age on interest in politics is moderated and as a consequence, indirect effects including this effect are moderated. This is called _moderated mediation_. In this example, sex is the moderator and interest in politics is the mediator.

```{r}
#| label: fig-causaldiagram5
#| fig-cap: "Causal diagram for interest in politics as mediator between age and newspaper reading time with sex as moderator of the effect of age on interest in politics."
#| echo: false
#| warning: false
