...
There is a very important way to minimize the chance of having any confounders at all, namely, randomization in an experiment. Remember the example of @sec-anova, where participants saw vaccination campaigns that used different language styles (controlling language, autonomy supportive language, and neutral language) to enhance vaccine acceptance. The experimental treatment here is the language style used in the campaign seen by each participant. If we let chance decide which campaign a participant sees, we randomize the experimental treatment. 

How does this help us to avoid having confounders? The example research aimed to find out whether the language style affects vaccine acceptance. The experimental treatment (language style of the campaign) is the independent variable or predictor variable in the model. If participants' scores on this variable --- in the example, seeing a campaign with controlling, autonomy supportive, or neutral language --- are random, the variable is expected not to correlate with any other characteristic of the participants when the experiment starts.

For example, participants with high and those with low health literacy would have the same chance to see a campaign with controlling, autonomy supportive, or neutral language style. We expect one third of all participants with high health literacy and one third of participants with low health literacy to see controlling language, to see autonomy supportive language, and to see neutral language. If there is no systematic difference between those with high and those with low health literacy in this respect, participant's experimental treatment is not correlated with health literacy of the participant. The same reasoning applies to every other characteristic of the participant at the start of the experiment: age, hair colour, political leaning, and so on. We expect that all of these participant characteristics are not correlated with the experimental treatment variable.
...
We have learned about probabilities and expectations in previous chapters. These principles also apply to experimental randomization. Even if we may expect to have equal numbers of those with high and low health literacy seeing a campaign with controlling language in our example experiment, we can end up with more people with high health literacy than people with low health literacy seeing controlling language in our experiment due to chance. In this situation, the experimental treatment variable is correlated with the health literacy of the participant, so participant health literacy is a confounder if it is also correlated with the dependent variable (vaccine acceptance) and not included in the analysis. Experimental randomization does not guarantee that there are no confounders but it is our best instrument to minimize the chance of having confounders. 
