...
The flow chart in figure @fig-choice-diagram-copy helps you select the appropriate statistical test based on the type of dependent and independent variables. 
...
__Chapter @sec-probmodels: Probability Models: How Do I Get a Sampling Distribution?__
...
* Video @fig-SPSSbootstrap1: (ref:bootstrap1SPSS)
* Video @fig-SPSSbootstrap2: (ref:bootstrap2SPSS)
* Video @fig-SPSSExact1: (ref:Exact1SPSS)
* Video @fig-SPSSExact2: (ref:Exact2SPSS)

__Chapter @sec-param-estim: Estimating a Parameter: Which Population Values Are Plausible?__

(ref:conflevelSPSS) Setting the confidence level in SPSS.

* Video @fig-SPSSconflevel: (ref:conflevelSPSS)

__Chapter @sec-hypothesis: Testing a Null Hypothesis: Am I Right or Am I Wrong?__
...
* Video @fig-SPSSbinomial: (ref:binomialSPSS)
* Video @fig-SPSSchisq1: (ref:chisq1SPSS)
* Video @fig-SPSS1mean: (ref:1meanSPSS)

__Chapter @sec-power: Which Sample Size Do I Need? Power!__

(ref:cohendtitle) Obtaining Cohen's _d_ with SPSS.

* Video @fig-cohend: (ref:cohendtitle)

__Chapter @sec-anova: Moderation with Analysis of Variance__
...
* Video @fig-SPSS1way: (ref:1waySPSS)
* Video @fig-SPSS2way: (ref:2waySPSS)
* Video @fig-SPSSeta2: (ref:eta2SPSS)

__Chapter @sec-moderationcat: Regression Analysis And A Categorical Moderator__
...
* Video @fig-SPSSregsimple: (ref:regsimpleSPSS)
* Video @fig-SPSSregdummy2: (ref:regdummy2SPSS)
* Video @fig-SPSSregdummy: (ref:regdummySPSS)
* Video @fig-SPSSregassumpt: (ref:regassumptSPSS)
* Video @fig-SPSSregpred: (ref:regpredSPSS)
* Video @fig-SPSSregcatmod: (ref:regcatmodSPSS)
* Video @fig-SPSSregmodlines: (ref:regmodlinesSPSS)
* Video @fig-SPSSregSupport1: (ref:regSupport1SPSS)

__Chapter @sec-moderationcont: Regression Analysis With A Numerical Moderator__
...
* Video @fig-SPSSregcenter: (ref:regcenterSPSS)
* Video @fig-SPSSreglines2: (ref:reglines2SPSS)
* Video @fig-SPSSregSupport2: (ref:regSupport2SPSS)

__Chapter @sec-confounder: Regression Analysis And Confounders__

(ref:regconfoundSPSS) Identifying confounders with regression in SPSS.

* Video @fig-SPSSregconfound: (ref:regconfoundSPSS)

__Chapter @sec-mediation: Mediation with Regression Analysis__
...
* Video @fig-SPSSmediatpar: (ref:mediatparSPSS)
* Video @fig-SPSSmediatserial: (ref:mediatserialSPSS)
* Video @fig-SPSSmediatcov: (ref:mediatcovSPSS)
* Video @fig-SPSSpath: (ref:pathSPSS)

__Appendix: Variance / Association__
...
* Video @fig-SPSSLevene: (ref:SPSSLevene)
* Video @fig-SPSSTindep: (ref:SPSSTindep)
* Video @fig-SPSSTpaired: (ref:SPSSTpaired)
* Video @fig-SPSScorrelation: (ref:SPSScorrelation)
* Video @fig-SPSSchisqcross: (ref:SPSSchisqcross)
...
For a binomial test, see the video in Figure @fig-SPSSbinomial. A one-sample chi-squared test is explained in the video of Figure @fig-SPSSchisq1.
...
The one-sample _t_ test in SPSS is explained in Figure @fig-SPSS1mean.
...
# The output in Figure @fig-Levene-output shows that the polarization (variation) in attitudes towards immigrants is not the same among young and old voters, _F_ = 4.99, _p_ = .029. Polarization is stronger among old voters (_SD_ = 2.06) than among young voters (_SD_ = 1.26). 
# 
...
For an instruction and exercises on one-way analysis of variance, see Figure @fig-SPSS1way and Section @sec-onewaySPSS. For two-way analysis of variance, see Section @sec-twowaySPSS (instructions and exercises).
...
For regression analysis (instructions and exercises), see Section @sec-SPSS-regression.
...
The sample outcome can be a single mean, for instance the average weight of candies, but it can also be the difference between two means, for example, the difference in colourfulness of yellow candies at the beginning and end of a time period. In the latter case, the standard deviation that we need is the standard deviation of colourfulness difference across all candies (Section @sec-dependentsamples). In the case of independent samples, such as average weight of red versus yellow candies, we need a special combined (*pooled*) standard deviation for yellow and red candy weight that is not reported by SPSS. Here, we use the *t* value and degrees of freedom to calculate Cohen's *d*.
